...
if(isset($_POST['reg']))
{
	...
	if($name!='' && $email!='' && $pwd!='' && $cpwd!='')
	{
		...
		{
			$epwd=md5($pwd);
			//echo "insert into tbl_user (name,email,password,cpassword) values('$name','$email,'$epwd','$pwd')";
			$ins_qry=$conn->query("insert into adminregister1 (name,email,password,cpassword) values('$name','$email','$pwd','$cpwd')");
			if($ins_qry)
			{
...
<html lang="en">
 ...
 <?php }?>
  ...
  <p>Already Have Account? <a href="adminlogin.php">Login</a></p>
</form>
  </div>
</div>
			</div>
